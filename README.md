![docker pulls](https://img.shields.io/docker/pulls/agaveplatform/jupyter-notebook.svg) ![docker stars](https://img.shields.io/docker/stars/agaveplatform/jupyter-notebook.svg) [![](https://images.microbadger.com/badges/image/agaveplatform/jupyter-notebook.svg)](https://microbadger.com/images/agaveplatform/jupyter-notebook "agaveplatform/jupyter-notebook image metadata")

# Jupyter Notebook Data Science Stack + Agave Tooling  

This will build the Jupyter datascience notebook image with the Agave Python SDK, Agave CLI, and several sample notebook files. It is meant to run as a local image in a development environment, or via a secure tunnel on a remote system as an Agave app.
...
* Agave R & Python SDK
* Agave CLI
* Quickstart guides and sample notebooks in Python 2, Python 3, R, and Bash
...
* `-e AGAVE_CACHE_DIR=/home/jovyan/work/.agave` - The folder where the Agave cache file will be written and accessed by the Agave tooling. 
* `-e AGAVE_JSON_PARSER=jq` - The JSON parser that will be used by the Agave CLI. Options are `jq`, `python`, `json-mirror`, and `native`.
* `-e AGAVE_TENANT=${AGAVE_TENANT}` - The code of the Agave tenant you wish you use from within the notebook. 
* `-e AGAVE_USERNAME=${AGAVE_USERNAME}` - An Agave username. If provided, this will be used to instead of prompting for a username within the notebooks. If not changed, the username will be read from your host environment.
* `-e AGAVE_PASSWORD=${AGAVE_PASSWORD}` - An Agave user password. If provided, this will be used to instead of prompting for a password within the notebooks. If not changed, the password will be read from your host environment. 
* `-e VM_HOSTNAME=localhost` - Sets the externally accessible hostname of the container. This allows for accurate url generation in the tutorials.       
